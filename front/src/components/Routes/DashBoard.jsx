...
export default function Dashboard({ gyms = [] }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const url = selectedGym
          ? `${API_URL}/gyms/dashboard/analytics?gymName=${encodeURIComponent(selectedGym)}`
          : `${API_URL}/gyms/dashboard/analytics`;
  ...
  const predictions = useMemo(() => {
    ...
    const hours = Object.keys(hourlyTotal)
      .map(Number)
      .sort((a, b) => a - b);
  ...
  return (
    <div className="dashboard-container">
      <div className="dashboard-header">
        ...
        <p>
          Análise estatística e acompanhamento de fluxo de alunos em tempo real usando dados reais
          dos dispositivos de catraca IoT.
        </p>
        <div className="gym-filter" style={{ marginTop: "16px" }}>
          <label htmlFor="gym-select" style={{ marginRight: "10px", fontWeight: "bold" }}>
            Select Gym:
          </label>
          <select
            id="gym-select"
          ...
          >
            ...
            {gyms.map((g) => (
              <option key={g._id || g.id || g.name} value={g.name}>
                {g.name}
              </option>
            ))}
          </select>
      ...
      <div className="prediction-section">
        <div className="prediction-card">
          <h4 className="best-hours-title">
            Melhores horários para {selectedGym || "todas as academias"}
          </h4>
          <p className="prediction-text">{predictions.empty || "No data"}</p>
        </div>
        <div className="prediction-card packed">
          <h4 className="packed-hours-title">
            Horários de pico para {selectedGym || "todas as academias"}
          </h4>
      ...
      <div
        className="chart-box"
        style={{ background: "#111", border: "1px solid #333", marginBottom: "3rem" }}
      >
        <h3 style={{ color: "#eaeaea", textAlign: "center" }}>
          Forecasting: Expected Demand Trend
        </h3>
        <ResponsiveContainer width="100%" height={300}>
          <AreaChart
            data={predictions.forecastData}
        ...
        >
          {(selectedGym ? [selectedGym] : gyms.map((g) => g.name)).map((gymName) => (
            <div
              key={gymName}
              className="chart-box"
              style={{ background: "#111", border: "1px solid #333", padding: "1rem", margin: 0 }}
            >
              <h4 style={{ color: "#eaeaea", textAlign: "center", marginBottom: "1rem" }}>
                {gymName}
              </h4>
              <iframe
