...
router.post("/", authenticateToken, async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Error creating gym, Gym may already exist", error: error.message });
  }
});
...
router.get("/dashboard/analytics", authenticateToken, async (req, res) => {
  try {
    ...
    const evolucao_hora = Object.keys(gymHourSum)
      .map((key) => {
    ...
    if (checkinsPerHourList.length > 0) {
      ...
      mediana_hora =
        checkinsPerHourList.length % 2 !== 0
          ? checkinsPerHourList[mid]
          : (checkinsPerHourList[mid - 1] + checkinsPerHourList[mid]) / 2;
    }
