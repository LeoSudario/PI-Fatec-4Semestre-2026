...
import { Animated, StyleSheet, Text, View, Alert, TouchableOpacity, Image } from "react-native";
...
export default function OccupancyBar({ gym, children, onDelete }: Props) {
  const safeCapacity = typeof gym.capacity === "number" && gym.capacity > 0 ? gym.capacity : 50;
  const safeCurrent = Math.max(0, typeof gym.occupancy === "number" ? gym.occupancy : 0);
  const progress = Math.max(0, Math.min(1, safeCurrent / safeCapacity));
  const percentage = progress * 100;
  const barColor = percentage < 50 ? "#4CAF50" : percentage < 90 ? "#FF9800" : "#EE3235";
  const effectColor = percentage < 50 ? "#388E3C" : percentage < 90 ? "#F57C00" : "#B71C1C";
